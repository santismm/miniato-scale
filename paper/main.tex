...
\date{September 2026 \quad\textperiodcentered\quad Preprint, version 1.0 \quad\textperiodcentered\quad Rules version \texttt{pilot-1.0}}
...
Calibrating mortality and economic loss in two separate tables is the obvious design, and it was the first one tried during development; the implied exchange rate between the tables then varied by an order of magnitude from one level to the next without anyone having decided it should. The scale therefore fixes a single declared anchor,
\begin{equation}
...
\caption{Aligned floors of the two numeric routes (rules \texttt{pilot-1.0}). Implied ratio at each boundary: $10^{7}$\,\euro\ per harm unit.}
\label{tab:routes}
\begin{tabular}{@{}ccc@{}}
\toprule
...
\node[draw=black!60,rounded corners=4pt,fill=white,inner sep=8pt,anchor=south west,align=left,text width=6.4cm] (card) at (7.2,1.6) {
 ...
 \footnotesize Rules: pilot-1.0 \quad Cut-off: 2026-08-26
};
...
\small
\paragraph{Data and code.} No personal data or victim records were used. The reference implementation (\texttt{miniato.py}, rules \texttt{pilot-1.0}), its tests and the case records are available at \url{https://github.com/santismm/miniato-scale}, version-tagged to match this manuscript. Case classifications in Table~\ref{tab:cases} are based solely on the cited public documents.
\paragraph{AI assistance.} This work was developed by the author through an explicitly multi-model, AI-assisted process, and that process is declared here as part of the method. Three large language models were used as instruments for critique, formalisation, reference search, implementation and drafting: Google Gemini 3.6 Flash (initial survey of existing severity scales and first draft of a scale), OpenAI GPT-5.6 Sol (critique of the earliest draft, design of the multidimensional record and of the first 0--10 index, an intermediate manuscript and its reference implementation, the first retrospective applications, and the naming search), and Anthropic Claude Fable 5.1 (critique of that intermediate manuscript, recalibration, the present manuscript, the reference implementation and the project documentation). The division of roles is as recorded by the author from the session histories. The models are not authors: the problem statement, every normative choice ($\theta$, $w$, the level anchors, the terminal conditions), the case classifications and the decision to publish are the author's, as is responsibility for all errors.
...
\paragraph{Versioning.} This is the first public version of the proposal, with rules \texttt{pilot-1.0}. It was preceded by internal development drafts that were reviewed and revised before release; where a draft's mistake illustrates a rule, the text says so without further detail. Future changes to any scoring rule will change the rules identifier; changes of prose alone will not.
...
\section{Domain rubrics with anchor examples (pilot-1.0)}
\label{app:rubrics}
...
\begin{table}[H]
\centering\small
\caption{Minimum Miniato record (pilot-1.0).}
\begin{tabularx}{\textwidth}{@{}l X@{}}
\toprule
Field & Requirement \\
\midrule
