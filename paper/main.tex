...
\date{September 2026 \quad\textperiodcentered\quad Preprint, version 2.4 \quad\textperiodcentered\quad Rules version \texttt{pilot-0.3}}
...
\begin{table}[!t]
\centering\small
\caption{Retrospective application (rules \texttt{pilot-0.3}). Profiles give the determining domain; braces denote admissible-level sets.}
\label{tab:cases}
...
\paragraph{Versioning.} This is version 2.4 of the proposal, with rules \texttt{pilot-0.3}. Version 2.4 answers a third review: the R1/R2 boundary now turns on whether an adverse decision took effect, not on whether a person reviewed it, because P1 classifies consequences; the ``four regions'' fact is attributed to the source that states it; the abstract lists the case results instead of implying a maximum; and the Spanish summary distinguishes loss of control during the incident from containment at the cut-off. Because a rubric criterion changed, the rules identifier changed. Version 2.3 reconciled text with results after a second external review: it withdraws a comparative statement between the Tempe and 2026 cases that the published classifications no longer supported, replaces ``non-fatal'' by ``no physical harm reported in the sources examined'', defines the output/decision boundary between R1 and R2, assigns each fact of the 2026 case to the source that documents it, and states that the two prior severity-rating efforts are linked. Version 2.2 responded to a first external methodological review: it makes the economic bands proportional to $\theta$ so that \eqref{eq:f} stays a partition under sensitivity analysis, replaces the claimed inequality with the EU AI Act by a subject-matter mapping, removes an unsupported upper bound on $F$ in the 2026 case, shows resolved rather than headline levels on the card, defines the bulletin's counting unit and dates, demotes the Gutenberg--Richter slope to an exploratory statistic, splits paired rubric bands, defines the state components, and discusses the prior severity ratings of \citet{mylius2025} and the MIT tracker. Versions 1.0 and 2.0 were named AIRA-10 (\emph{AI Incident Rating \& Assessment}); the name was changed because that acronym is already used by several unrelated AI risk-assessment frameworks, and because a public scale is better served by a proper name, as with Richter, Mercalli, Torino and Palermo. Rules \texttt{pilot-0.2} (versions 2.0 to 2.3) differ from \texttt{pilot-0.3} only in the R1/R2 criterion; the numeric routes are unchanged. Version 1.0 (rules \texttt{pilot-0.1}) used separate mortality and economic tables with inconsistent implied ratios, placed the first fatality at level 5, lacked an evaluation-contained rule, and did not mandate the control flag. Those changes are documented in Section~\ref{sec:calibration} and Table~\ref{tab:cases}.
